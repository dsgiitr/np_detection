...
vehicles_list=["car","bus","truck"] # vehicles with number plate (motorbike excluded)
...
with open(output_file) as json_file:
    ...
    for i_frame in range(len(data)): # for each image in input_file
        vehicle_in_image=False
        in_file=data[i_frame]["filename"]
        print(f"for {i_frame+1} image :") 
        list_objects=data[i_frame]["objects"]
        list_objects=[i["name"] for i in list_objects]
        for obj in list_objects:
            if obj in vehicles_list:
                vehicle_in_image=True
        if vehicle_in_image:
            print("save this frame")
            img = Image.open(in_file)
            out_file = 'output/'
            img.save(out_file+in_file[5:])
